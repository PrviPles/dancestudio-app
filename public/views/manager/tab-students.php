 ...
 * UPGRADED with full AJAX functionality and responsive data-labels.
 */
        ...
        <tbody>
            ...
            <?php else :
                ...
                <tr class="dsa-student-row" id="dsa-student-row-<?php echo esc_attr($student->ID); ?>">
                    <td class="dsa-full-name" data-label="<?php esc_attr_e('Full Name', 'dancestudio-app'); ?>"><?php echo esc_html($full_name); ?></td>
                    <td class="dsa-age" data-label="<?php esc_attr_e('Age', 'dancestudio-app'); ?>"><?php echo ($bday = get_user_meta($student->ID, '_dsa_user_birth_date', true)) ? dsa_calculate_age($bday) : '—'; ?></td>
                    <td class="dsa-email" data-label="<?php esc_attr_e('Email', 'dancestudio-app'); ?>"><?php echo esc_html($student->user_email); ?></td>
                    <td class="dsa-phone" data-label="<?php esc_attr_e('Phone', 'dancestudio-app'); ?>"><?php echo esc_html(get_user_meta($student->ID, '_dsa_user_phone', true) ?: '—'); ?></td>
                    <td class="dsa-partner" data-label="<?php esc_attr_e('Partner', 'dancestudio-app'); ?>"><?php echo ($pid = get_user_meta($student->ID, 'dsa_partner_user_id', true)) ? esc_html(get_the_author_meta('display_name', $pid)) : '—'; ?></td>
                    <td class="dsa-groups" data-label="<?php esc_attr_e('Active Groups', 'dancestudio-app'); ?>"><?php echo !empty($active_groups) ? esc_html(implode(', ', $active_groups)) : '—'; ?></td>
                    <td class="dsa-actions" data-label="<?php esc_attr_e('Actions', 'dancestudio-app'); ?>">
...
<div id="dsa-edit-student-modal" style="display:none;" title="<?php esc_attr_e('Edit Student Details', 'dancestudio-app'); ?>">
    <form id="dsa-edit-student-form">
        ...
        <table class="form-table">
            ...
            <tr><th colspan="2"><h3><?php esc_html_e('Dance Studio Info', 'dancestudio-app'); ?></h3></th></tr>
