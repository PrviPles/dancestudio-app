...
<div class="dsa-profile-form-wrapper">
    ...
    <form id="dsa-profile-form" action="<?php echo esc_url(admin_url('admin-post.php'));?>" method="post" enctype="multipart/form-data">
        ...
        <p><label for="dsa_phone_number"><?php esc_html_e('Mobile Phone','dancestudio-app');?></label><br/><input type="tel" name="phone_number" id="dsa_phone_number" value="<?php echo esc_attr(get_user_meta($current_user_id,'_dsa_user_phone',true));?>"/></p>
        <p><label for="dsa_student_birthday"><?php esc_html_e('Birthday','dancestudio-app');?></label><br/><input type="date" name="dsa_student_birthday" id="dsa_student_birthday" value="<?php echo esc_attr(get_user_meta($current_user_id,'_dsa_user_birth_date',true));?>"/></p>
        
        <hr><h3><?php esc_html_e('Discounts','dancestudio-app');?></h3>
        <p class="description"><?php esc_html_e('For discounts contact studio administrator', 'dancestudio-app'); ?></p>
        <p>
            <label for="dsa_is_retired">
                <input type="checkbox" name="dsa_is_retired" id="dsa_is_retired" value="1" <?php checked( '1', get_user_meta( $current_user_id, '_dsa_is_retired', true ) ); ?> disabled="disabled">
                <?php esc_html_e('I am a retiree', 'dancestudio-app'); ?>
            </label>
        </p>
        <p>
            <label for="dsa_family_discount">
                <input type="checkbox" name="dsa_family_discount" id="dsa_family_discount" value="1" <?php checked( '1', get_user_meta( $current_user_id, '_dsa_family_discount', true ) ); ?> disabled="disabled">
                <?php esc_html_e('I am eligible for a family discount', 'dancestudio-app'); ?>
            </label>
        </p>
