...
if ( ! function_exists( 'dsa_render_private_lessons_tab_content' ) ) {
    function dsa_render_private_lessons_tab_content( $student_data ) {
        ...
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th><?php _e('Lesson Date');?></th>
                    <th><?php _e('Lesson Title');?></th>
                    <th><?php _e('Teacher');?></th>
                    <th><?php _e('Figures Practiced');?></th>
                    <th><?php _e('Linked Order');?></th>
                </tr>
            </thead>
            <tbody>
            <?php if ( ! empty( $lessons ) ) : foreach( $lessons as $lesson ) :
                ...
                if (!empty($figure_ids) && is_array($figure_ids)) {
                    foreach ($figure_ids as $figure_id) {
                        $figure_title = get_the_title($figure_id);
                        if ($figure_title) {
                            $figure_names[] = $figure_title;
                        }
                    }
                }
            ?>
            ...
            <?php endforeach; else: ?>
                <tr><td colspan="5"><?php esc_html_e('This student has no private lesson history.', 'dancestudio-app'); ?></td></tr>
            <?php endif; ?>
            </tbody>
        </table>
        <?php
