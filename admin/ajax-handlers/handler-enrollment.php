...
function dsa_handle_enrollment_ajax_submission() {
    ...
    // --- Process the requested sub-action ---
    ...
    if ( 'enroll' === $sub_action ) {
        ...
        // On success, create the HTML for the new table row to send back to the browser
        $student_data = get_userdata( $student_id );
        $enroll_date = get_post_meta( $result, '_dsa_enrollment_date', true );
        
        // Build the HTML row dynamically
    ...
    // If no valid sub_action was provided
    wp_send_json_error( ['message' => 'Invalid action specified.'] );
}

/**
 * Handles updating a user's enrollment in a single group via AJAX from their profile page.
 */
...
function dsa_update_user_enrollment_ajax_handler() {
    ...
    if ( $is_enrolled ) {
        // If the checkbox was checked, enroll the student
        ...
        // If the checkbox was unchecked, drop them out
...
/**
 * Handles fetching all groups and a specific student's enrollments for the modal.
 */
...
function dsa_get_enrollment_modal_data_handler() {
    ...
    // 2. Get the groups this specific student is enrolled in
