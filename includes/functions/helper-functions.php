...
function dsa_generate_hub3a_barcode_string( $order, $studio_settings ) {
    // This is a private helper function just for the barcode string
    function _dsa_sanitize_for_barcode($string) {
        $croatian_chars = ['č', 'ć', 'ž', 'š', 'đ', 'Č', 'Ć', 'Ž', 'Š', 'Đ'];
        $ascii_replacements = ['c', 'c', 'z', 's', 'd', 'C', 'C', 'Z', 'S', 'D'];
        $string = str_replace($croatian_chars, $ascii_replacements, $string);
    ...
    // Sanitize data ONLY for the barcode
    $lines[] = _dsa_sanitize_for_barcode(strtoupper(substr($order->get_formatted_billing_full_name(), 0, 25)));
    $lines[] = _dsa_sanitize_for_barcode(strtoupper(substr($order->get_billing_address_1(), 0, 25)));
    $lines[] = _dsa_sanitize_for_barcode(strtoupper(substr($order->get_billing_city(), 0, 25)));
    $lines[] = _dsa_sanitize_for_barcode(strtoupper(!empty($studio_settings['studio_name']) ? substr($studio_settings['studio_name'], 0, 25) : ''));
    $lines[] = _dsa_sanitize_for_barcode(strtoupper(!empty($studio_settings['street_address']) ? substr($studio_settings['street_address'], 0, 25) : ''));
    $lines[] = _dsa_sanitize_for_barcode(strtoupper(!empty($studio_settings['zip_code']) && !empty($studio_settings['city']) ? substr($studio_settings['zip_code'] . ' ' . $studio_settings['city'], 0, 25) : ''));
