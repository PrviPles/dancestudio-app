...
function dsa_get_studio_statistics_data( $filters = [] ) {
    ...
    foreach( $all_classes as $class ) {
        ...
        $end_time_str = get_post_meta($class->ID, '_dsa_class_end_time', true);
        
        if ($class->post_type === 'dsa_private_lesson' && empty($end_time_str)) {
             $end_time_str = date('H:i:s', strtotime($start_time_str) + 45 * 60);
        }
        ...
        if ($class->post_type === 'dsa_group_class') {
            ...
            if(is_array($attendance_data)){
                foreach($attendance_data as $student_id => $data){
                    if ( isset($data['status']) && in_array($data['status'], $statuses_that_count) ) {
                        $attendees[] = $student_id;
                    }
                }
            }
            $class_group_id = get_post_meta($class->ID, '_dsa_class_group_id', true);
            if ($class_group_id && !isset($group_attendance[$class_group_id])) {
                $group_attendance[$class_group_id] = 0;
            } // **SYNTAX FIX**: Added missing curly braces for clarity.
            if ($class_group_id) {
                $group_attendance[$class_group_id] += count($attendees);
            }
        } else {
