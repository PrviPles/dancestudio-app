 ...
 * UPDATED: Adds logic to initialize subscription tracking fields upon enrollment.
 *
...
function dsa_enroll_student_in_group( $student_id, $group_id, $enroll_date = '' ) {
    ...
    if ( ! empty( $enroll_date ) ) {
        $enrollment_data['post_date'] = $enroll_date;
    }
    ...
    // --- NEW: Initialize subscription tracking fields when a student is enrolled ---
    update_post_meta( $record_id, '_dsa_subscription_start_date', '' ); // Starts empty
    update_post_meta( $record_id, '_dsa_lessons_attended_count', 0 );
    update_post_meta( $record_id, '_dsa_current_subscription_order_id', '' ); // Starts empty
